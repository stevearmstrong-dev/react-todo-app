...
const getSortValue = (task: Task, dayStart: Date): number => {
  ...
  if (task.scheduledStart) {
    const scheduleDate = new Date(task.scheduledStart);
    if (getStartOfDay(scheduleDate).getTime() === dayStart.getTime()) {
      return scheduleDate.getHours() * 60 + scheduleDate.getMinutes();
    }
  }
...
const UpcomingView: React.FC<UpcomingViewProps> = ({ tasks, onToggleComplete, onDeleteTask, onFocus }) => {
    ...
    <div className="upcoming-view">
      ...
      <div className="upcoming-days">
        {days.map((day) => {
          ...
          const dayTasks = (tasksByDay[key] || []).sort(
            (a, b) => getSortValue(a, day) - getSortValue(b, day)
          );

          return (
            <div className="upcoming-day" key={key}>
